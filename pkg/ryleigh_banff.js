/* @ts-self-types="./ryleigh_banff.d.ts" */
 ...
 * @returns {Uint8Array}
 */
export function oil_paint(bytes, width, height) {
    const ptr0 = passArray8ToWasm0(bytes, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.oil_paint(ptr0, len0, width, height);
 ...
 * @returns {Uint8Array}
 */
export function render(width, height, seed, kick) {
    const ret = wasm.render(width, height, seed, kick);
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('ryleigh_banff_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
